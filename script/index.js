...
const apiSearch = (event) => {
	...
	if(searchText.trim().length === 0) {
		modalText.innerHTML = 'The search box should not be blank!';
		modalWindow.style.display = "block";
		return;
	}
	movie.innerHTML = '<div class="spinner"></div>';
	fetch(`https://api.themoviedb.org/3/search/multi?api_key=20ea6a54518a25a3b110a6646d33a414&language=en&query=${searchText}`)
		.then((value) => {
			if (value.status !== 200) {
...
document.addEventListener('DOMContentLoaded', () => {
	movie.innerHTML = '<div class="spinner"></div>';
	fetch('https://api.themoviedb.org/3/trending/all/week?api_key=20ea6a54518a25a3b110a6646d33a414&language=en')
		.then((value) => {
			if (value.status !== 200) {
				return Promise.reject(value);
			}
		...
		.then((output) => {
			let inner = '<p class="popular-week">Popular for the week</p>';
			if (output.results.length === 0 || output.results[0].media_type === 'person') {
				modalText.innerHTML = "Your search did't match anything.";
				modalWindow.style.display = "block";
			};
